...
import { buildDifferential } from "./TopGearEngineShared";
...
describe("The stat weight path reports no HPS rather than a fabricated one", () => {
  ...
  test("the count of embellished items selected is reported", () => {
    expect(typeof result.embellishedSelected).toEqual("number");
    expect(result.embellishedSelected).toBeGreaterThanOrEqual(0);
  });
});

describe("buildDifferential", () => {
  const makeSet = (hardScore, setHPS) => ({ hardScore, setHPS, itemList: [], enchantBreakdown: { Gems: [] } });

  test("carries absolute HPS and the gap against the best set", () => {
    const differential = buildDifferential(makeSet(99000, 445500), makeSet(100000, 450000), null, "Raid");
    expect(differential.hps).toEqual(445500);
    expect(differential.hpsDifference).toEqual(-4500);
  });

  test("reports zero when the sets have no modelled throughput", () => {
    const differential = buildDifferential(makeSet(99000, 0), makeSet(100000, 0), null, "Raid");
    expect(differential.hps).toEqual(0);
    expect(differential.hpsDifference).toEqual(0);
  });
});
